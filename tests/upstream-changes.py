...
path_to_sync_script = {
    ...
    "applications/kserve/models-web-app/upstream": "scripts/synchronize-kserve-web-application-manifests.sh",
...
path_to_sync_script = {Path(k): Path(v) for k,v in path_to_sync_script.items() }

def find_upstream_scripts(changed_files: list[Path]) -> set[Path]:
    upstream_scripts = set()
    for changed in changed_files:
        for upstream_path, script in path_to_sync_script.items():
...
if __name__ == "__main__":
    ...
    if args.all:
        upstream_scripts = set(path_to_sync_script.values())
    else:
    ...
    for script in sorted(upstream_scripts):
        ...
        process = subprocess.Popen(
            ...
            env={"KUBEFLOW_SYNC_NO_COMMIT": "true"},
            text=True,
        )
